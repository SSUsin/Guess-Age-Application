...
export default class DropArea extends React.Component<IProps, IState>{
    ...
    public onDrop(files: any) {
        ...
        try{
            ...
            this.props.setResults("❗️ Sorry, we had trouble loading that file please use a downloaded image file. ❗️",0);
        }
    }
            ...
            .then((response: any) => {
                if (!response.ok) {
                    ...
                    response.json().then((json: any[]) => {
                        if(json.length<1){
                            this.props.setResults("❗️ Sorry, No face detected. ❗️",this.state.imageFiles.length)
                        }else{
                            ...
                            <div className="dropZoneText">
                                {
                                    this.state.imageFiles.length > 0 ?
                                        ...
                                        <p>Try dropping some files here, or click to select files to upload. 🗂️</p>
                                }
                            </div>
                        </ReactDropzone>
                    </div>
